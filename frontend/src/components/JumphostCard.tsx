...
import { useJumphostTrafficHistory } from '@/api/jumphosts'
import type { Jumphost } from '@/api/jumphosts'
import { Shield } from 'lucide-react'

interface JumphostCardProps {
  jumphost: Jumphost
...
export default function JumphostCard({ jumphost, mask }: JumphostCardProps) {
  const { data: traffic } = useJumphostTrafficHistory(jumphost.status === 'online' ? jumphost.id : null)
  const m = mask || ((v: string | null | undefined) => v || '')

  const rates = traffic?.rates || []
  const sparkData = rates.map(r => r.rx_rate + r.tx_rate)

  const stats = jumphost.system_stats
    ...
    <div className="glass rounded-lg p-4 hover:border-primary/25 transition-all duration-300 group">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2 min-w-0">
          <h3 className="font-semibold text-sm group-hover:text-primary transition-colors truncate">{jumphost.name}</h3>
          {jumphost.hardened && (
            <span title="Hardened"><Shield className="w-3.5 h-3.5 text-emerald-400/60 flex-none" /></span>
          )}
        </div>
        <StatusBadge status={jumphost.status} message={jumphost.status_message} />
      </div>
      <p className="text-xs text-muted-foreground mt-1 font-mono truncate">{m(jumphost.ip)}</p>

      {/* Protocol port */}
      <div className="mt-3 flex gap-4 text-xs text-muted-foreground">
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-1.5 h-1.5 rounded-full bg-amber-400/60" />
          SS:{jumphost.shadowsocks_port || '—'}
        </span>
      </div>
      ...
      {/* Footer */}
      {jumphost.last_health_check && (
        <p className="text-xs text-muted-foreground/60 mt-2.5">
          {formatRelativeTime(jumphost.last_health_check)}
        </p>
      )}
      {jumphost.status_message && (
        <p className="text-xs text-orange-400/70 mt-1 truncate">{jumphost.status_message}</p>
      )}
    </div>
  )
}
